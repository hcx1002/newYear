<template>
	...
	>
		<tm-sheet :margin="[0,0]" :padding="[0,0]" :black="black_tmeme" :round="0" 
		color=""
			:classname="(flat?'':`  py-${padding[1]} `)+` ${borderBottom?black_tmeme?'border-grey-darken-5-b-1 ':'border-grey-lighten-4-b-1':''}`"
			:shadow="0">
			<view @click="onclickInput" :class="[vertical?'tm-input-col':'flex-between ']" :style="{
				...
				<view v-if="leftIcon||title" class="tm-input-left flex-start flex-shrink" :class="[vertical?'pb-24':'']">
					<!-- icon -->
					<view class="pr-16 vertical-align-middle flex-center">
						<tm-icons dense v-if="leftIcon" :name="leftIcon" :color="color_tmeme"></tm-icons>
					</view>
					<!-- 标题 -->
				...
				<view  class="flex-between " :class="[disabled?'opacity-6':'',]" style="width: 100%;">
					...
					<view class="tm-input-center relative fulled" >
						<view class="flex-start   tm-input-center-wk"
							...
							`border-${black_tmeme?(borderColor?'grey-darken-2':''):borderColor}-a-1`,
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 校验是否通过。
			verifyInput() {
				let verify = this.verify.call(this, this.valdata||'');
				// #ifdef H5 || APP-VUE
				verify = verify(this.valdata||'')
				// #endif
